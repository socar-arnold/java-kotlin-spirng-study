...
## Phase 종료 미니 퀴즈 (필수 — 적극적 회상)
Phase 마지막 세션이 끝나면 *그 자리에서* 미니 퀴즈로 회상 확인. 커밋 전에 진행.
- **분량:** 3~5문항, 10분 이내
- **구성:** 회상 2 + 응용 1~2 + ⚠️함정 1 + 🔭스트레치 1(다음 Phase 예고)
- **양식:** 학습자가 텍스트로 직접 답 → 시니어 코드리뷰 스타일 피드백
- **저장:** 그 Phase 학습 노트 끝에 "퀴즈 + 답 + 피드백" 섹션 추가 → 통합 복습 시 자산
- **시점:** Phase 마지막 세션 결과 GREEN/정상 동작 확인 직후, PROGRESS 갱신·커밋 *전*

> Stage 종료 통합 복습(여러 Phase 묶음 회고)과 헷갈리지 말 것. 작은 점검은 매 Phase, 큰 점검은 매 Stage.
