...
## Phase CS — 면접·실전 빈출 CS (병렬 트랙)

> **성격:** 전 구간에 걸쳐 **매 세션 1꼭지(~10분)** 곁들이는 병렬 트랙. 목표는 "**면접에서 말로 설명할 수준**" —
> 깊은 구현이 아니라 개념·트레이드오프를 또렷이 설명할 수 있으면 충분. 같은 주제를 코드로 **구현**하는 건
> 연계 Phase에서 (예: 인덱스 개념=여기, 인덱스 설계 실습=Phase 5). **면접·실전 빈출 우선**으로 선별했다.
> 레슨은 적시 집필 — 그 세션의 메인 학습과 가장 연관된 꼭지를 우선 배치하고, 없으면 순서대로.

### CS-1. 네트워크
- [ ] **TCP vs UDP** — 연결지향·신뢰성(순서/재전송) vs 비연결·속도. 언제 무엇을(파일전송 vs 스트리밍/게임)
- [ ] **HTTP/HTTPS·TLS 핸드셰이크** — 대칭키 vs 비대칭키, 인증서가 보장하는 것
- [ ] **HTTP 1.1 / 2 / 3** — keep-alive, 멀티플렉싱, HOL(Head-of-Line) blocking, QUIC(UDP 기반)
- [ ] **쿠키 vs 세션 vs 토큰(JWT)** — 상태를 어디에 저장하나, 확장성·보안 트레이드오프
- [ ] **CORS** — 브라우저 동일출처정책, 왜 막고 어떻게(Preflight) 푸나
- 연계: Phase 4(백엔드 HTTP/REST), 4-F(JWT)

### CS-2. 운영체제
- [ ] **프로세스 vs 스레드** — 메모리 공유 여부, 생성 비용
- [ ] **컨텍스트 스위칭** — 비용은 어디서 오나
- [ ] **가상 메모리·페이징** — 물리/가상 주소, 스왑
- [ ] **동기화·데드락** — 임계구역, 데드락 4조건, 회피/예방
...
- [ ] **정렬** — 퀵/머지/힙, 안정 정렬 여부
- 연계: Phase 1-D(컬렉션), Phase 5-A(인덱스=B-tree)

### CS-4. DB CS
- [ ] **인덱스 원리(B-tree)** — 왜 빠른가, 언제 안 타나
- [ ] **트랜잭션·ACID** — 원자성/일관성/격리성/지속성
- [ ] **격리 수준** — Dirty/Non-repeatable/Phantom read, 수준별 트레이드오프
- [ ] **정규화 vs 반정규화** — 중복 제거 vs 조회 성능
- 연계: Phase 5(DB 심화), Phase 4-C(JPA)

### 세션 페어링 가이드 (예시)
| 메인 학습 | 곁들일 CS |
|---|---|
| Phase 1-D 컬렉션 | CS-3 해시테이블·Big-O |
| Phase 3 동시성 | CS-2 프로세스/스레드·데드락 |
| Phase 4 백엔드 | CS-1 HTTP/TCP·쿠키/세션/토큰 |
| Phase 5 DB | CS-4 인덱스·트랜잭션·격리수준 |

> 진척은 위 체크박스로 표시(완료 시 `[x]`). 메인 학습과 안 겹치는 시기엔 CS-1→2→3→4 순서로 1꼭지씩.
